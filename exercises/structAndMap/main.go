...
var userOrderedSL []user

func exportAsOrderedSlice() []user {
	keys := keyOrderer()
	for _, k := range keys {
		userOrderedSL = append(userOrderedSL, userHM[k])
	}
	return userOrderedSL
}

// ancillary functions
// sort and print the users Hashmap
func printUserHM() {
	fmt.Println("The users hashmap are:")
	keys := keyOrderer()
	for _, k := range keys {
...
func main() {
	...
	addUser(us1) // instead of userHM[us1.id] = us1, but with validity check
	addUser(us2)
