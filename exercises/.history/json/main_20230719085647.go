...
func main() {
	...
	prt("5 + 3 =", calc.Calculator.Add(c, 5, 3))      // 8
	prt("5 - 9 =", calc.Calculator.Sub(c, 5, 9))      // -4
	prt("22 * 33 =", calc.Calculator.Mult(c, 22, 33)) // 726
	prt("4 / 0 =", calc.Calculator.Divide(c, 4, 0))   // impossible to divide by zero
	prt("12 / 6 =", calc.Calculator.Divide(c, 12, 6)) // 2
	prt()
	// _ = calc.Calculator.EncodeHistory()
	calc.Calculator.SaveToFile(c, calc.Calculator.EncodeHistory(c), "test.json")
	...
	calc.Calculator.SaveToFile(c, History2, "test2.json")
	calc.Calculator.DecodeHistory(c, "test2.json")
	calc.Calculator.AddToHistory(c, "test2.json")
	calc.Calculator.PrintHistory(c, )
}
