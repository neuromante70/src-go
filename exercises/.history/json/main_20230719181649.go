...
func (c calculator.Calculator) Add(num1 int, num2 int) int {
	op := calculator.MathRequest{num1, "Add", num2}
	c.History = append(c.History, op)
	return num1 + num2
}

func (c calculator.Calculator) Sub(num1 int, num2 int) int {
	op := calculator.MathRequest{num1, "Sub", num2}
	c.History = append(c.History, op)
	return num1 - num2
}

func (c calculator.Calculator) Mult(num1 int, num2 int) int {
	op := calculator.MathRequest{num1, "Mult", num2}
	c.History = append(c.History, op)
	return num1 * num2
}

func (c calculator.Calculator) Divide(num1 int, num2 int) int {
	if num2 == 0 {
		prt("impossible to divide by zero")
		return 0
	}
	op := calculator.MathRequest{num1, "Div", num2}
	c.History = append(c.History, op)
	return num1 / num2
}
...
func (c calculator.Calculator) EncodeHistory() (eh []byte) {
...
func (c calculator.Calculator) SaveToFile(eh []byte, fn string) {
	f, err := os.Create(fn)
	if !check(err) {
		return
	}
...
func (c calculator.Calculator) DecodeHistory(fn string) (f []byte) {
...
func (c calculator.Calculator) AddToHistory(fn string) {
	mr := make([]MathRequest, 10)
	f, err := os.ReadFile(fn)
	if !check(err) {
		return
	...
	for _, mRequest := range mr {
		c.History = append(c.History, calculator.MathRequest(mRequest))
	}
	// for i := 0; i < len(mr); i++ {
...
// ancillary function
func (c calculator.Calculator) PrintHistory() (history []string) {
...
func main() {
	...
	prt("5 + 3 =", calculator.Calculator.Add(5, 3))      // 8
