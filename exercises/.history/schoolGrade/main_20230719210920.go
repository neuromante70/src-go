...
func main() {
	...
	ex2 := exam{2, 2, 1, 3} // exNr 2, Janis, math, superior
	...
	ex9.extPrint()
	grd, _ := s1.avgGrade()
	prt("the average of student", Students[1], " is ", grd) // Carl grades average = 2.66
	prt("calculating Carl grades average for math (2)")
	prt(s1.avgGradeBySbj(1)) // Carl grades average for math = 2
	// grd, _ = s2.avgGrade()
	// prt("the average of student", Students[2], " is ", grd) // Carl grades average = 2.75
	grd, _ = s2.avgGrade()
	prt("the average of student", Students[2], " is ", grd) // Janis grades average = 3.6
	prt("calculating Janis grades average for science (2)")
	prt(s2.avgGradeBySbj(2)) // Janis grades average for science = 2
	// prt(Exams)
}
