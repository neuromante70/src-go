...
func prtAvgBySbj(avg int, err error) {
	if err != nil {
		prt(err)
		return
	}
...
func main() {
	...
	v := prtAvgBySbj(s1.avgGrade())
	grd, _ := s1.avgGrade()
	prt("the grades average of student", Students[1], " is ", grd) // Carl grades average = 2.66
	v, _ = s1.avgGradeBySbj(1)
	prt("calculating Carl grades average for math:", v) // Carl math average = 2
	grd, _ = s2.avgGrade()
	prt("the grades average of student", Students[2], " is ", grd) // Janis grades average = 3
	v, _ = s2.avgGradeBySbj(2)
	prt("calculating Janis grades average for science:", v) // Janis grades average for science = 3
	grd, _ = s3.avgGrade()
	prt("the grades average of student", Students[3], " is ", grd) // Luise grades average = 4
	v, _ = s3.avgGradeBySbj(3)
	prt("calculating Luise grades average for science:", v) // Janis grades average for science = 3
	v, err = s3.avgGradeBySbj(1)
	prt("calculating Luise grades average for math:", v, err) // Janis grades average for science = 3
}
