...
func (s student) avgGrade() (avg int, err error) {
	...
	avg = int(math.Floor(float64(sum) / float64(k)))
	return avg, nil
}
...
func (s student) avgGradeBySbj(sbj int) (avg int, err error) {
	var sum int
	k := 1
	for _i, v := range Exams {
		if v.idStd == s.id {
			// Exams = map[int]exam{}
			// prt("this is exams key", i, "this is studId ", s.id)
			if v.subject == sbj {
				// prt("this is exams subject", v.subject)
				sum = sum + v.grade
				// prt("this is sum", sum)
				// prt("this is index", i)
			}
	...
	if k == 1 {
		// prt("i at the end of the loop", k)
...
func main() {
	...
	prt("calculating Janis grades average for math = 2")
