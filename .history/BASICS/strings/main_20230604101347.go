...
func main() {
	...
	s2 = `w la figa!`
	fmt.Println("s2 After the modification: s2 ", s2)
	fmt.Println("s2 and s: s2 ", s2, "s:", s)
	fmt.Println(&s, &s2)
	var culo interface{}
	culo = "interface type"
	fmt.Println("Before the modification: ", culo)
	culo = 25
	fmt.Println("After the modification: ", culo)
}
