...
func main() {
	...
	fmt.Println(len(userHM))

	userSL := make([]user, len(userHM)=1)
	for i := 0; i <= len(userHM) -1; i++ {
		userSL[i] = user{id: userHM[i].id, name: userHM[i].name, email: userHM[i].email}
		// userSL[i].name = userHM[i].name
		// userSL[i].email = userHM[i].email
	}
	fmt.Println(userSL)
}
