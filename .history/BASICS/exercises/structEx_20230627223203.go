...
func exportAsOrderedSlice() []user {
	...
	this woks if the slice is declared inside the function
	for key, user := range userHM {
		userSL[key-1] = element
	} */
	var keys []int
	for k := range userHM {
		keys = append(keys, k)
...
func main() {
	...
	fmt.Println("this is the hashmap:")
	printUserHM()

	userSlice := exportAsSlice()
	fmt.Println("and his is the slice:")
	printUserSL(userSlice)
	userSlice = exportAsOrderedSlice()
	fmt.Println("and his is the ordered slice:")
	printUserSL(userSlice)
}
