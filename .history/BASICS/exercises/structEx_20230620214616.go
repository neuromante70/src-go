...
func main() {
	...
	if err != nil {
		...
		fmt.Println(fndUs1.name, "exist")
	}

 this user is not in db 
	// the user (us5) is not present in the db and therefore the program will return an error
	us5 := user{id: 4, name: "John Smith", email: "smith@yahoo.com"}
	fndUs2, err := findUser(us5)
	if err != nil {
		fmt.Println(err.Error())
	} else {
		fmt.Println(fndUs2.name, "exist")
	}
