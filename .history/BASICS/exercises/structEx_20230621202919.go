...
func addUser(us user) bool {
	if _, exist := userExist(us.id); !exist {
		userHM[us.id] = us
		fmt.Println("The user wasn't present in userHM and now is correctly inserted")
		return true
	}
	fmt.Println("The user is already present in userHM and now is correctly inserted")	
	return false
}
