...
func main() {
	...
	// this user (3) is not already added in hashMap
	...
	// this user (4) is already present in hashMap
	...
	// this user (us1) exist in hashMap
	fndUs1, err := findUser(us1)
	if err != nil {
		fmt.Println(err.Error())
	} else {
		fmt.Println(fndUs1.name, "user present in hashMap")
	}

	// the user (us5) is not present in the hashMap, hence the program will return an error
	us4 := user{id: 4, name: "John Smith", email: "smith@yahoo.com"}
	fndUs2, err := findUser(us4)
	if err != nil {
		fmt.Println(err.Error())
	} else {
		fmt.Println(fndUs2.name, "user present in hashMap")
	}

	addUser(us4)
	fmt.Println(hashMap, us4)
	deleteUser(us4)
	fmt.Println(hashMap)
