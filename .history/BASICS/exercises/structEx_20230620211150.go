...
func findUser(us user) bool {
	if userExist(us) {
		//fmt.Println(us.id, us.name, us.email)
		return true
	}
		return false
	}
...
func main() {
	...
	us2 := user{id: 1, name: "Jenny Connelly", email: "connelly@gmail.com"}
	//db = map[int]user
	db[us1.id] = us1
	db[us2.id] = us2
	...
	*/
	fndUs1 := findUser(us1)
	if fndUs1 {
		fmt.Println(us1.id, us1.name, us1.email)
	}
}
